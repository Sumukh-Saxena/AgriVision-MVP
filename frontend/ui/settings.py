...
def render_settings() -> None:
    ...
    with st.expander("How it works"):
        st.markdown(
            "1. **Upload** a leaf/crop image (JPG, JPEG, or PNG) and enter your "
            "city/state.\n"
            "2. **Classify** - a CNN model predicts the disease and its confidence.\n"
            "3. **Weather** - OpenWeather fetches local conditions for your location.\n"
            "4. **Explain** - if confidence is above 50%, a LangGraph workflow asks "
            "Mistral AI for a structured, farmer-friendly explanation that accounts "
            "for the local weather.\n"
            "5. **Verify** - for low-confidence results, the app recommends manual "
    ...
    with st.expander("Technical details (for developers)"):
        st.markdown(
            ...
            "- Weather: OpenWeather API (`worker/weather.py`)\n"
